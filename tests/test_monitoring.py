...
async def test_start_monitoring_scheduler_prefers_temporal_schedule(monkeypatch):
    calls = []
    ...
    async def _fake_ensure(interval):
        ...
        return scheduler.get_monitoring_scheduler_status()
    ...
    status = await scheduler.start_monitoring_scheduler(interval_seconds=300)
...
async def test_start_monitoring_scheduler_falls_back_when_temporal_raises(monkeypatch):
    ...
    # state, which would break later tests (e.g. stop_monitoring_scheduler) if left in place.
    scheduler._task = None
    scheduler._stop_event = None
