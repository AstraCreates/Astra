...
export default function RedesignSidebar({ mobile = false, open = false, onClose }: { mobile?: boolean; open?: boolean; onClose?: () => void } = {}) {
  ...
  const navStyle: React.CSSProperties = mobile
    ? { width: 240, maxWidth: "82vw", display: "flex", flexDirection: "column", background: "var(--surface)", borderRight: "1px solid rgba(255,255,255,0.55)", height: "100dvh", boxShadow: "4px 0 32px rgba(0,46,255,.10), 2px 0 8px rgba(0,0,0,.10)", position: "fixed", top: 0, left: 0, zIndex: 60, transform: open ? "translateX(0)" : "translateX(-104%)", transition: "transform 0.28s cubic-bezier(0.22, 1, 0.36, 1)", backdropFilter: "blur(32px) saturate(1.4)", WebkitBackdropFilter: "blur(32px) saturate(1.4)" }
    : { width: 210, flexShrink: 0, display: "flex", flexDirection: "column", background: "var(--surface)", borderRight: "1px solid rgba(255,255,255,0.55)", height: "100vh", boxShadow: "4px 0 32px rgba(0,46,255,.10), 1px 0 0 rgba(255,255,255,0.6)", position: "sticky", top: 0, zIndex: 3, backdropFilter: "blur(32px) saturate(1.4)", WebkitBackdropFilter: "blur(32px) saturate(1.4)" };
