...
export default function RedesignSidebar({ mobile = false, open = false, onClose }: { mobile?: boolean; open?: boolean; onClose?: () => void } = {}) {
  ...
  const navStyle: React.CSSProperties = mobile
    ? { width: 240, maxWidth: "82vw", display: "flex", flexDirection: "column", background: "rgba(255,255,255,0.72)", borderRight: "1px solid rgba(255,255,255,0.85)", height: "100dvh", boxShadow: "4px 0 40px rgba(0,46,255,.08), 2px 0 12px rgba(0,0,0,.06), inset -1px 0 0 rgba(255,255,255,0.9)", position: "fixed", top: 0, left: 0, zIndex: 60, transform: open ? "translateX(0)" : "translateX(-104%)", transition: "transform 0.28s cubic-bezier(0.22, 1, 0.36, 1)", backdropFilter: "blur(40px) saturate(1.8)", WebkitBackdropFilter: "blur(40px) saturate(1.8)" }
    : { width: 210, flexShrink: 0, display: "flex", flexDirection: "column", background: "rgba(255,255,255,0.72)", borderRight: "1px solid rgba(255,255,255,0.85)", height: "100vh", boxShadow: "4px 0 40px rgba(0,46,255,.08), inset -1px 0 0 rgba(255,255,255,0.9)", position: "sticky", top: 0, zIndex: 3, backdropFilter: "blur(40px) saturate(1.8)", WebkitBackdropFilter: "blur(40px) saturate(1.8)" };
