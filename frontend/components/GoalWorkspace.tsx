...
function NewGoalOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  const router = useRouter();
  const { userId: devUserId, isSignedIn } = useDevUser();
      ...
      <LiquidGlass style={{ width: "min(1120px, 100%)", maxHeight: "calc(100vh - 48px)" }} contentStyle={{ padding: "28px 30px", display: "flex", flexDirection: "column", gap: 18, maxHeight: "calc(100vh - 48px)", overflowY: "auto" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 10, paddingTop: 4 }}>
            {!isSignedIn && (
              <a href="/api/auth/signin/google" style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--fg-dim)", padding: "8px 14px", borderRadius: 8, border: "1px solid var(--line)", background: "transparent", textDecoration: "none" }}>
            ...
            <button type="submit" disabled={loading || !instruction.trim() || !isSignedIn} className="site-btn site-btn-primary" style={{ padding: "0 24px", fontSize: 14, opacity: isSignedIn ? 1 : 0.4 }}>
...
function WorkspaceSidebar({
  ...
  const { isSignedIn, user } = useDevUser();
  ...
  return (
    <div className="site-card" style={{ minWidth: 0, padding: 14, display: "flex", flexDirection: "column", gap: 14, minHeight: "100%" }}>
      ...
      {isSignedIn ? (
        <button data-tour="new-goal-btn" type="button" onClick={onNewGoal} className="site-btn site-btn-primary" style={{ width: "100%", minHeight: 42, justifyContent: "flex-start", padding: "0 14px", fontSize: 13 }}>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        ...
        {isSignedIn ? (
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 4px" }}>
            ...
            <button onClick={() => fetch("/api/auth/signout", { method: "POST" }).then(() => location.reload())}
