        ...
        <div style={{ flexShrink: 0, position: "relative", zIndex: 5, padding: "14px 30px", borderBottom: "1px solid var(--bd)", background: "linear-gradient(90deg, rgba(37,99,235,.07), var(--bg) 62%)", boxShadow: "0 10px 24px rgba(2,6,23,.12)" }}>
          {activeBuilds.slice(0, 1).map(({ squad, task }) => (
            <div key={`pinned-build-${task.id}`} style={{ maxWidth: 820, margin: "0 auto", border: "1px solid var(--accent)", borderRadius: 12, padding: 12, background: "var(--bg-surface)" }}>
              ...
              {task.previewUrl && isSafeHttpUrl(task.previewUrl) ? <div style={{ border: "1px solid var(--bd)", borderRadius: 8, overflow: "hidden" }}><div style={{ padding: "6px 9px", fontSize: 11, background: "var(--bg-sunken)", display: "flex", justifyContent: "space-between" }}><span>Live preview</span><a href={task.previewUrl} target="_blank" rel="noreferrer" style={{ color: "var(--accent)" }}>Open site ↗</a></div><iframe src={task.previewUrl} title="Live website preview" sandbox="allow-scripts allow-same-origin allow-forms" style={{ width: "100%", height: 300, border: 0, display: "block" }} /></div> : <div style={{ marginTop: 9, color: "var(--fm)", fontSize: 11 }}>Preview is waiting for the local server to become healthy.</div>}
