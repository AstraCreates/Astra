  ...
  return <div style={{ display: "flex", height: "100%", minHeight: "100vh", background: "var(--bg)" }}>
    ...
    <main style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", height: "100vh" }}>
      ...
      {activeBuilds.length > 0 && (
        <div style={{ flexShrink: 0, position: "relative", zIndex: 5, padding: "14px 30px", borderBottom: "1px solid var(--bd)", background: "linear-gradient(90deg, rgba(37,99,235,.07), var(--bg) 62%)", boxShadow: "0 10px 24px rgba(2,6,23,.12)" }}>
          {activeBuilds.slice(0, 1).map(({ squad, task }) => (
            <div key={`pinned-build-${task.id}`} style={{ maxWidth: 760, margin: "0 auto", border: "1px solid var(--accent)", borderRadius: 12, padding: 10, background: "var(--bg-surface)" }}>
              ...
              {task.previewUrl && isSafeHttpUrl(task.previewUrl) ? <div style={{ border: "1px solid var(--bd)", borderRadius: 8, overflow: "hidden" }}><div style={{ padding: "6px 9px", fontSize: 11, background: "var(--bg-sunken)", display: "flex", justifyContent: "space-between" }}><span>Live preview</span><a href={task.previewUrl} target="_blank" rel="noreferrer" style={{ color: "var(--accent)" }}>Open site ↗</a></div><iframe src={task.previewUrl} title="Live website preview" sandbox="allow-scripts allow-same-origin allow-forms" style={{ width: "100%", height: 220, border: 0, display: "block" }} /></div> : <div style={{ marginTop: 9, color: "var(--fm)", fontSize: 11 }}>Preview is waiting for the local server to become healthy.</div>}
              <div style={{ marginTop: 10 }}>{task.terminalSessionId ? <TerminalPane sessionId={task.terminalSessionId} compact onClose={() => undefined} /> : <div style={{ color: "var(--fm)", fontSize: 11, display: "flex", alignItems: "center", gap: 7 }}><Loader2 size={12} className="company-home-spin" /> Allocating the shared coding terminal…</div>}</div>
            </div>
          ))}
        </div>
      )}
                            ...
                            return (
                              <div className="ch-chat-bubble" style={{ maxWidth: openTerminalTask || hasLivePreview ? "min(640px, 92%)" : "min(420px, 84%)" }}>
                                ...
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 12, gap: 10 }}>
                                  ...
                                  {sitesVisited > 0 && <small style={{ color: "var(--fm)", fontSize: 11 }}>{sitesVisited} website{sitesVisited === 1 ? "" : "s"} visited</small>}
                                </div>
      ...
      <aside className="company-home-rail" style={{ width: railWidth, flexShrink: 0, height: "100vh", overflowY: "auto", borderLeft: "1px solid var(--bd)", padding: "18px 16px 30px", background: "var(--bg-sunken)" }}>
        ...
        <section style={{ marginBottom: 22 }}>
          ...
          {home.approvals.length === 0 ? (
            ...
            <div style={{ display: "grid", gap: 8 }}>{home.approvals.map(item => {
              const busy = approvingId === item.id;
              return <article key={item.id} style={{ padding: 11, border: "1px solid var(--ab)", borderRadius: "var(--radius)", background: "var(--adim)" }}><div style={{ display: "flex", gap: 6, color: "var(--amber)" }}><ShieldCheck size={13} /><b style={{ fontSize: 11.5, color: "var(--fg)" }}>{item.title}</b></div><p style={{ margin: "6px 0", fontSize: 10.5, color: "var(--fd)" }}>{item.detail}</p><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}><small style={{ color: "var(--amber)", fontSize: 10 }}>{item.squad}</small><button type="button" disabled={busy} onClick={() => {
                // The decision route pays the same full-snapshot cost every
                // other Company OS write does -- with no disabled/pending
                // state here, a slow response reads as "nothing happened",
                // and clicking again double-submits (confirmed live: the
                // same approval recorded three separate decisions minutes
                // apart). Guard by id, same convention as messageBusyId.
                if (approvingId) return;
                setApprovingId(item.id);
                void decideCompanyApproval({ founderId, companyId }, item.id, true)
                  .then(setHome)
                  .catch(error => setNotice(friendlyErrorMessage(error, "Approval failed.")))
                  .finally(() => setApprovingId(""));
              }} style={{ padding: "6px 9px", border: 0, borderRadius: 6, background: "var(--accent)", color: "#fff", cursor: busy ? "default" : "pointer", fontSize: 10, opacity: busy ? 0.6 : 1 }}>{busy ? "Approving…" : "Approve"}</button></div></article>;
            })}</div>
          )}
        </section>
