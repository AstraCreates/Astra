...
/**
 * AgentSwarm — solar-system orbital model.
 *
 * Sun        = company name (center)
 * Planets    = active department agents, orbiting the sun
 * Moons      = sub-specialist agents (research_X, technical_X, …), orbiting their planet
 * Inactive   = hidden; pop in with a spring animation on first activation
 * Done       = keep orbiting, color switches to mint
 */
...
interface Props {
  ...
  coreLabel: string;
  coreSub?: string;
  height?: number; // ignored — aspect ratio is fixed
}
...
// Orbit radii (inner → outer), one per potential planet
const ORBIT_R  = [88, 122, 154, 183, 210, 234, 256, 275];
// Orbital period in seconds (inner planets faster)
const ORBIT_T  = [12, 17, 22, 28, 34, 41, 48, 56];
// Moon period in seconds (all moons same-ish speed, staggered)
const MOON_T_BASE = 3.2;

function clip(s: string, n = 14) { return s.length > n ? s.slice(0, n - 1) + "…" : s; }

/** Group flat agent list into planet/moon hierarchy by key prefix. */
...
export default function AgentSwarm({ agents, coreLabel, coreSub }: Props) {
  // Angle state lives in refs — mutated in RAF without triggering renders.
  ...
  // Filter to agents that are visible (running / done / error). Waiting = not shown.
  ...
  // Initialise angles for newly-appearing planets/moons (evenly spread).
  useEffect(() => {
  ...
  // RAF loop — advance all angles proportional to elapsed time.
  useEffect(() => {
  ...
  // Compute Cartesian positions from current angles — reruns on every frame tick.
  ...
  const positions = useMemo(() => groups.map(({ dept, planet, moons }, i) => {
    ...
    // Label: push radially outward from the sun
  ...
  }), [groups, frame]); // frame dependency forces recompute every RAF tick
  ...
  return (
    <div style={{
      ...
      >
        ...
        {/* ── Moon orbit tracks (follow the planet, re-rendered each frame) ── */}
        ...
        {positions.map(p => {
          ...
          return (
            <g key={p.dept}>
              ...
              {/* Planet disc — spring pop-in via SVG animate on first appear */}
              <circle cx={p.px} cy={p.py} r={PLANET_R}
                fill={c} opacity={isRunning ? 0.95 : 0.85}
                filter={isRunning ? "url(#sw-glow)" : undefined}
              >
                {/* r: 0 → overshoot → settle — plays once when element is created */}
                <animate attributeName="r"
              ...
              {/* Label — radially outward, orbits with planet */}
              <text
                x={p.lx} y={p.ly}
                textAnchor={p.anchor} dominantBaseline="middle"
                fontSize={8.5}
                fill={isRunning ? "var(--blue)" : isDone ? "var(--mint)" : "var(--fd)"}
      ...
      {/* Sun label (HTML for crisp subpixel text) */}
      <div style={{
      ...
      <div style={{
        ...
        {doneCount > 0 && (
          <span style={{ color: "var(--mint)", fontWeight: 500 }}>{doneCount} done</span>
        )}
