...
def build_technical_agent(**kwargs) -> Agent:
    return Agent(
        ...
        role=(
            ...
            "   (a) COHESIVE / medium-size product work: run_mvp_loop(\n"
            "         repo_url=<repo>,\n"
            ...
            "       ). This is your medium-brain path: use it for reasoning-heavy implementation work, cohesive "
            "features, and integrated fixes.\n"
            "   (b) LARGER product (2+ cleanly independent areas) — BUILD IN PARALLEL: spawn_parallel_coders(repo_url=<repo>, "
            "modules=[{name, goal, owns}, ...], context=<research notes>). Each module gets its OWN LIGHT coding agent "
            "running concurrently while you remain the supervising technical agent; 'owns' is the directory that module "
            "owns (NON-OVERLAPPING across modules, e.g. 'frontend/app/(auth)', 'frontend/app/dashboard', "
            "'frontend/lib/billing'). The tool merges the modules, then you review/fix the integrated result. "
            "Use this whenever you can break work into isolated chunks such as auth flows, dashboard sections, "
            "and targeted bugfixes. Same design-preservation + auth rules apply to every module.\n"
            ...
            "7. done — return {repo_url, deploy_url, files_in_repo, tech_stack: [list of technology names used], mvp_roadmap, technical_plan}. "
            "mvp_roadmap must be a concrete prioritized build sequence with user stories, milestones, and acceptance criteria. "
            "technical_plan must describe the implemented architecture, data model, auth, API routes, deployment, and tradeoffs. "
            "These required artifacts must contain substantive text, not only URLs or a summary. "
            ...
            "Prefer the pattern: medium model handles planning/integration, light worker coders handle isolated modules. "
