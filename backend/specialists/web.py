...
def build_web_agent(**kwargs) -> Agent:
    ...
    return Agent(
        ...
        role=(
            ...
            "     goal='COMING SOON launch page for <COMPANY_NAME>: a polished Next.js + Tailwind "
            "\"coming soon\" page — hero with company name, sharp tagline, and 2-3 sentence value prop; "
            "a brief \"what we're building\" section with 3-4 benefit bullets; an email waitlist form with "
            "a CTA like \"Join the waitlist\" or \"Get early access\" — this is the ONLY CTA, there is NO "
            "link to a product app (it does not exist yet); an FAQ with 3-4 honest Q&As. "
            "COMING SOON badge or ribbon visible in the hero. NO fabricated testimonials, customer logos, "
            "user counts, or made-up metrics. Use the brand colors and fonts from the design spec.',\n"
            "     required_files=['package.json','next.config.js','tailwind.config.ts','app/layout.tsx',"
            "'app/globals.css','app/page.tsx','components/Hero.tsx','components/Waitlist.tsx'],\n"
            "     session_id=<SESSION>,\n"
            "     context=<design colors/fonts + 2-3 sentence product summary>\n"
            "   ) — builds the Next.js coming-soon page, deploys it, and returns repo_url + deploy_url.\n"
            "4. obsidian_log — log the repo_url AND deploy_url clearly (the technical agent extends THIS repo).\n"
            "5. done — return {repo_url, deploy_url, landing_page, website_copy}. "
            "landing_page must describe the implemented page, sections, routes, deploy URL, and QA status. "
            "website_copy must contain the actual hero, benefits, CTA, FAQ, and other copy used in the build. "
            "Do not return only URLs or a short summary.\n\n"
