...
def _build_queries(topic: str, agent_name: str) -> list:
    ...
    if base in ("research", "research_market"):
        return [
            ...
            f'"{t}" market size revenue TAM analysis',
            f'"{t}" funding investment venture capital',
            f'"{t}" industry news 2024 2025',
            f'"{t}" industry report analyst research',
            f'"{t}" market share leading companies revenue',
            f'"{t}" barriers entry challenges startup',
            f'"{t}" B2B enterprise customer segments',
            f'"{t}" unit economics SaaS metrics',
            ...
            f'"{t}" investor report outlook forecast',
            f'"{t}" competitive landscape map players',
            f'"{t}" use cases customer success stories',
            f'"{t}" market opportunity gap whitespace',
            ...
            f'"{t}" growth driver trend forecast outlook',
        ]

    if base == "research_financial":
        return [
            f'"{t}" funding rounds investors raised capital',
            f'"{t}" venture capital Series A B C funding',
            f'"{t}" valuation latest funding round 2024 2025',
            f'"{t}" investor portfolio companies',
            f'"{t}" revenue ARR SaaS business model',
            f'"{t}" IPO exit acquisition valuation',
            f'"{t}" venture return analysis investor ROI',
            f'"{t}" fundraising timeline milestones burn rate',
            f'"{t}" cap table dilution equity financing',
            f'"{t}" angel investors seed funding network',
            f'"{t}" venture debt financing growth funding',
            f'"{t}" revenue benchmarks retention metrics',
            f'"{t}" unit economics growth multiple CAC LTV',
            f'"{t}" exit strategies acquisition targets',
            f'"{t}" venture market size funding available',
            f'"{t}" investor check size round size stage',
            f'"{t}" due diligence financials investor questions',
            f'"{t}" salary equity compensation employees',
            f'"{t}" dilution antidilution rights preferences',
            f'"{t}" venture thesis investor focus criteria',
            f'"{t}" fundraising deck pitch investors',
            f'"{t}" portfolio company performance metrics',
            f'"{t}" term sheet valuation economics',
            f'"{t}" venture capital concentration risk',
            f'"{t}" financing gap bridge round convertible',
            f'"{t}" venture return outlook fund performance',
            f'"{t}" investor confidence market sentiment',
            f'"{t}" comparable acquisitions valuation multiples',
            f'"{t}" minority stake preference structure',
            f'"{t}" venture opportunity size addressable market',
        ]

    if base == "research_regulatory":
        return [
            f'"{t}" regulatory compliance requirements legal',
            f'"{t}" government policy regulations industry laws',
            f'"{t}" licensing requirements permits compliance',
            f'"{t}" data privacy GDPR CCPA compliance',
            f'"{t}" SOC2 compliance certification security',
            f'"{t}" industry regulations standards compliance',
            f'"{t}" tax implications business structure',
            f'"{t}" employment law hiring contractors',
            f'"{t}" intellectual property patent trademark',
            f'"{t}" product liability insurance requirements',
            f'"{t}" contract law terms conditions privacy',
            f'"{t}" international expansion regulatory requirements',
            f'"{t}" import export compliance tariffs trade',
            f'"{t}" financial services regulations FINRA SEC',
            f'"{t}" healthcare HIPAA compliance medical device',
            f'"{t}" environmental compliance sustainability',
            f'"{t}" labor law compliance wage hour overtime',
            f'"{t}" anti-corruption FCPA bribery compliance',
            f'"{t}" accessibility compliance ADA WCAG',
            f'"{t}" antitrust competition law restrictions',
            f'"{t}" consumer protection law FTC regulations',
            f'"{t}" legal entity formation Delaware C-Corp',
            f'"{t}" escrow agreement dispute resolution',
            f'"{t}" regulatory filing requirements SEC',
            f'"{t}" compliance timeline deadline milestones',
            f'"{t}" audit requirements external auditors',
            f'"{t}" regulatory risk matrix assessment',
            f'"{t}" legal counsel startup law firms',
            f'"{t}" compliance automation tools frameworks',
            f'"{t}" regulatory intelligence monitoring changes',
        ]

    if base == "research_competitors":
        return [
            f'"{t}" companies platforms list 2024 2025',
            f'"{t}" startups funding raised capital',
            f'"{t}" alternatives competitors comparison',
            f'"{t}" best tools market leaders',
            f'"{t}" Y Combinator YC startup',
            f'"{t}" top companies players landscape',
            f'"{t}" pricing plans cost comparison',
            f'"{t}" market map named competitors',
            f'"{t}" vs comparison feature comparison',
            f'"{t}" reviews customer feedback sentiment',
    ...
    return [
        ...
        f'"{t}" user pain points discussion forum',
        f'"{t}" regulatory compliance requirements startup',
        f'"{t}" sales strategy channels B2B B2C',
        f'"{t}" founder interview how we built',
        f'"{t}" first 100 customers growth strategy',
        f'"{t}" pricing strategy models free trial',
        f'"{t}" product market fit signals indicators',
        f'"{t}" SaaS metrics LTV CAC unit economics',
        f'"{t}" content marketing SEO growth strategy',
        f'"{t}" paid acquisition performance ROI channels',
        f'"{t}" hiring early team roles recruitment',
        f'"{t}" retention churn reduction onboarding',
        f'"{t}" infrastructure scaling cloud architecture',
        f'"{t}" security compliance GDPR SOC2',
        f'"{t}" partnership integration channel sales',
        f'"{t}" investor pitch deck metrics fundraising',
        f'"{t}" competitive moat defensibility advantage',
        f'"{t}" community building early adopters',
        f'"{t}" startup lessons case study success',
        f'"{t}" case study ROI success metrics',
        f'"{t}" youtube tutorial build launch',
        f'"{t}" API integration third party tools',
        f'"{t}" database architecture backend choices',
        f'"{t}" frontend framework design system',
        f'"{t}" viral loop referral product led',
        f'"{t}" startup failure lessons what not to do',
    ]
