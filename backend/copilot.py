...
async def run_copilot(
    ...
    system = (
        ...
        "STATUS ACCURACY: before reporting a team/department (research, design, etc.) as 'done', 'completed', or "
        "'finished', check EVERY individual agent in that team against the agents[] array in the snapshot — "
        "e.g. research is research_market + research_competitors + research_customers + research_gtm + "
        "research_execution, not one agent. If ANY of them has status 'error', the team is NOT done: say exactly "
        "which sub-agent failed and why (its summary usually names the failure, e.g. max_iterations_reached), "
        "and do not claim the team's work is complete or safe to build on until the founder decides whether to "
        "retry it or proceed without it. Never round a partial success up to 'done' — a mix of done+error agents "
        "is a partial result, not a finished one.\n\n"
