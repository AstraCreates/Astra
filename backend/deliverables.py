...
_DEPT_BY_KEYWORD: list[tuple[str, str]] = [
    (("legal", "compliance", "contract"), "Legal"),
    (("market", "marketing", "brand", "outreach", "seo", "ads"), "Marketing"),
    (("research", "competitor", "analysis", "survey"), "Research"),
    (("finance", "revenue", "stripe", "payment", "billing"), "Finance"),
    (("ops", "operations", "support", "customer"), "Ops"),
    (("tech", "engineer", "code", "deploy", "infra"), "Technical"),
]


def _infer_dept(label: str) -> str:
    low = label.lower()
    for keywords, dept in _DEPT_BY_KEYWORD:
        if any(k in low for k in keywords):
            return dept
    return "Research"


def sync_deliverables_to_library(
    founder_id: str,
    session_id: str,
    agent_label: str,
    result: dict[str, Any],
) -> None:
    """After a run completes, add each generated PDF/file to the founder's
    library so it shows up without the founder having to dig through sessions."""
    try:
        from backend.library.store import create_file, list_files
        paths = collect_result_attachment_paths(result)
        if not paths:
            return
        dept = _infer_dept(agent_label)
        existing = {f.get("source_path") for f in list_files(founder_id)}
        for p in paths:
            if str(p) in existing:
                continue
            content = f"Generated by {agent_label} · session {session_id[:8]}"
            create_file(
        ...
        logging.getLogger(__name__).warning(
            "sync_deliverables_to_library failed founder=%s: %s", founder_id, exc
        )
